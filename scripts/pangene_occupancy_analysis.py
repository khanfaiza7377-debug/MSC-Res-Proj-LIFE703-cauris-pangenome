...
# Canonical genome labels are resolved by pattern, not by exact column name, so the
# script works whichever GET_PANGENES run it is pointed at. The 2026 run that the
# dissertation reports uses GCF_002759435.1 for B8441 and Candida_auris_6684_fun for
# 6684; the earlier all-FungiDB run used FungiDB-68_CaurisB8441 / _Cauris6684.
GENOME_PATTERNS = [
    ('Cauris_B11220', r'B11220'),
    ('Cauris_B11221', r'B11221'),
    ('Cauris_B11243', r'B11243'),
    ('Cauris_B11245', r'B11245'),
    ('Cauris_B8441',  r'B8441|GCF_002759435|GCA_002759435'),
    ('Cauris_6684',   r'6684'),
]


def resolve_genome_columns(columns):
    """Map the matrix's actual column names onto canonical genome labels."""
    resolved, used = {}, set()
    for canon, pat in GENOME_PATTERNS:
        for col in columns:
            if col in used:
                continue
            if re.search(pat, str(col), re.I):
                ...
                used.add(col)
                break
    missing = [c for c, _ in GENOME_PATTERNS if c not in resolved.values()]
    if missing:
        raise SystemExit(
            "Could not find a column for: %s\nColumns present: %s"
            % (', '.join(missing), list(columns)))
    print("Genome columns resolved:")
    for col, canon in resolved.items():
        print("  %-32s -> %s" % (col, canon))
    return resolved
...
def load_pangene_matrix(path):
    ...
    resolved = resolve_genome_columns(df.columns)
    df = df.rename(columns=resolved)
    new_genome_cols = list(resolved.values())
...
def _norm_id(value):
    """Normalise a gene identifier for joining.

    RefSeq-derived matrices carry identifiers such as 'gene-B9J08_001532' or
    'rna-...', whereas FungiDB exports the bare locus tag 'B9J08_001532'. Without
    this the inner join below silently discards most genes.
    """
    if pd.isna(value):
        return None
    v = str(value).strip()
    v = re.sub(r'^(gene|rna|cds|id)[-:]', '', v, flags=re.I)
    v = re.sub(r'\.\d+$', '', v)          # trailing version suffix
    return v.upper() or None


def _pick_column(ref, *candidates):
    """Find a column by name, tolerating case and spacing differences."""
    lookup = {re.sub(r'[^a-z0-9]', '', c.lower()): c for c in ref.columns}
    for cand in candidates:
        hit = lookup.get(re.sub(r'[^a-z0-9]', '', cand.lower()))
        if hit:
            return hit
    return None
...
def merge_with_reference(pg, summary_path):
    ...
    for col in ['Protein Length', 'Ortholog count', 'Paralog count',
        ...
        if col in ref.columns:
            ...
            print("  ! reference table has no '%s' column" % col)
            ref[col] = np.nan

    # Expression columns are looked up by name, never by position: the column
    # order of a FungiDB export is not stable.
    for target, names in (
        ('expr_sensitive', ('Fluconazole sensitive', 'sensitive', 'expr_sensitive')),
        ('expr_resistant', ('Fluconazole resistant', 'resistant', 'expr_resistant')),
    ):
        hit = _pick_column(ref, *names)
        if hit is None:
            print("  ! no expression column matched %s - expression panel will be empty" % (names,))
            ref[target] = np.nan
        else:
            print("  expression column for %s: %r" % (target, hit))
            ref[target] = pd.to_numeric(ref[hit], errors='coerce')

    ipr = _pick_column(ref, 'Interpro ID', 'InterPro ID', 'Interpro Description')
    ref['has_interpro'] = (
        ref[ipr].apply(lambda x: not (pd.isna(x) or str(x).strip() in ['N/A', '', 'nan']))
        if ipr else False)
    prod = _pick_column(ref, 'Product Description')
    ref['is_annotated'] = (
        ref[prod].apply(lambda x: not (pd.isna(x) or str(x).strip() == 'hypothetical protein'))
        if prod else False)

    gid = _pick_column(ref, 'Gene ID', 'source_id', 'Gene')
    if gid is None:
        raise SystemExit("Reference table has no gene identifier column")

    pg = pg.copy()
    pg['_join'] = pg['Cauris_B8441'].apply(_norm_id)
    ref['_join'] = ref[gid].apply(_norm_id)

    merged = pg.merge(ref, how='inner', on='_join')

    eligible = pg['_join'].notna().sum()
    rate = (len(merged) / eligible * 100) if eligible else 0.0
    print("\nMerged {:,} of {:,} B8441 genes with reference annotations ({:.1f}%)"
          .format(len(merged), eligible, rate))
    if rate < 80:
        print("  ! WARNING: merge rate below 80%. Check that the identifier styles match -")
        print("    matrix example: %r" % (pg['Cauris_B8441'].dropna().iloc[0] if eligible else None))
        print("    reference example: %r" % (ref[gid].dropna().iloc[0] if len(ref) else None))
    return merged
