...
## Declaration of generative AI use — analysis code

All analysis code in this repository was written by me, and the scientific design, execution
and interpretation of every analysis are my own.

Claude (Anthropic) was used as a debugging assistant. In
[`scripts/pangene_occupancy_analysis.py`](scripts/pangene_occupancy_analysis.py) it identified
and fixed three defects — a genome-column mapping that no longer matched the final
GET_PANGENES run, an unguarded inner join that could silently discard most genes where
identifier styles differed, and expression columns read by position rather than by name — and
added the helper functions implementing those fixes. The remaining scripts in `scripts/` are
unmodified.

The job scripts [`slurm/run_liftoff.sh`](slurm/run_liftoff.sh) and
[`funannotate/run_funannotate_6684.sh`](funannotate/run_funannotate_6684.sh), and this README,
were drafted with Claude from my own HPC job scripts and run guides; the commands, parameters
and tool versions they record are those I used.I was also provided with scripts from my co-supervisor and supervisor.

No results, data or interpretations were generated by AI.
